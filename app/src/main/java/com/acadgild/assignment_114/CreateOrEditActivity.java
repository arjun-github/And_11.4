package com.acadgild.assignment_114;
...
public class CreateOrEditActivity extends ActionBarActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(personID > 0) {
            ...
            rs.moveToFirst();
            String personName = rs.getString(rs.getColumnIndex(MyDatabaseHelper.PERSON_COLUMN_NAME));
            String personGender = rs.getString(rs.getColumnIndex(MyDatabaseHelper.PERSON_COLUMN_GENDER));
            int personAge = rs.getInt(rs.getColumnIndex(MyDatabaseHelper.PERSON_COLUMN_AGE));
            if (!rs.isClosed()) {
                rs.close();
            }
